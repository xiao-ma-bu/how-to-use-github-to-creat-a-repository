...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "videomae_v5"
...
#    基于V4实际结果回退并稳定化：
#    ① 保留90~100%连续训练时间窗口
#    ② 保留中心附近3-Clip + 0.25/0.50/0.25加权Logits融合
#    ③ 保留sqrt Player-Class软平衡，但默认取消类别总质量1:1归一化
#    ④ 关闭Ordinal Soft Target与Label Smoothing，恢复Hard Cross Entropy
#    ⑤ Stage2分层学习率恢复到V3较保守配置
#    ⑥ 显式梯度裁剪max_grad_norm=1.0
#    ⑦ 每个Stage2验证Epoch保存逐视频/逐球员诊断并检测类别塌缩
#    ⑧ 保存split指纹，确保V5与后续实验使用同一数据划分
# ============================================================
RANDOM_SEED = 42
...
# V5恢复Hard Cross Entropy，不做Label Smoothing/Ordinal Soft Target
...
# 每次正式实验清理V5旧stage输出，避免误恢复旧checkpoint
...
# ============================================================
# 8. V5：Player-Class软平衡 + Hard Cross Entropy
# ============================================================
def build_player_class_sample_weights(dataset: BasketballVideoDataset) -> torch.DoubleTensor:
    ...
    print("\n" + "=" * 70)
    print("V5 Player-Class Soft Sampling")
    print("=" * 70)
...
class PlayerBalancedTrainer(Trainer):
    """V5：Hard Cross Entropy由Transformers默认模型loss负责，仅覆盖训练Sampler。"""
...
# ============================================================
# 11. Stage2：Layer-wise Learning Rate + V5中心加权Multi-Clip Validation
# ============================================================
...
class LayerwiseMultiClipTrainer(PlayerBalancedTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        if self.multi_clip_num_clips != 3:
            raise ValueError("V5中心加权Multi-Clip固定使用3个Clip")
        if len(self.multi_clip_weights) != 3:
            raise ValueError("V5 CENTER_CLIP_WEIGHTS必须包含3个权重")
    ...
    def evaluate(
        ...
        """
        V5训练期间每个Epoch使用中心附近3-Clip Validation：
        ...
        print(
            f"\n[V5 3-Clip Weighted VAL] epoch={self.state.epoch} "
...
def evaluate_multi_clip(
    ...
    print("\n" + "=" * 70)
    print("V5 3-Clip Center-Weighted 测试")
    print("=" * 70)

    if num_clips != 3:
        raise ValueError("V5测试固定使用3个中心附近Clip")
...
def main() -> None:
    ...
    print("=" * 70)
    print("VideoMAE V5 - Basketball Shot Type Classification")
    print("=" * 70)
    ...
    # ========================================================
    # Stage 2：Layer-wise LR + V5中心加权3-Clip Validation
    # ========================================================
    ...
    print(f"最佳V5 3-Clip加权VAL Macro-F1：{stage2_trainer.state.best_metric}")
    ...
    # ========================================================
    # 测试：保留Single-Clip用于对照 + V5中心加权3-Clip作为主结果
    # ========================================================
