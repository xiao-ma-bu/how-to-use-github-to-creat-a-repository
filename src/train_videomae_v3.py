...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "videomae_v3"
...
#    核心改动：
#    ① 连续时间窗口 + 窗口内均匀采样
#    ② Player-Class Balanced Sampler
#    ③ Stage2 Layer-wise Learning Rate
#    ④ 3-Clip Validation 直接参与最佳Checkpoint选择
# ============================================================
RANDOM_SEED = 42
...
# Player-Class Balanced Sampling
ENABLE_PLAYER_CLASS_BALANCED_SAMPLER = True

# 训练增强（整段视频使用同一组参数，避免帧间闪烁）
...
# 每次正式实验建议清理旧stage输出，避免误恢复旧checkpoint
...
def temporal_multiview_sample_frames(
    ...
    """
    真正的Temporal Multi-View：
    每个clip对应一个连续时间窗口，各窗口沿时间轴均匀移动；
    每个窗口内部再均匀采样num_frames帧。
    """
    total = len(frames)
    if total <= 0:
        raise RuntimeError("视频没有有效帧")
...
# ============================================================
# 8. Player-Class Balanced Sampler
# ============================================================
def build_player_class_sample_weights(dataset: BasketballVideoDataset) -> torch.DoubleTensor:
    """
    权重设计：
      每个class总采样权重约相等；
      同一class内部，每个player-class组总权重约相等。

    对样本i：
      weight_i = 1 / (该player-class样本数 × 该class的player-class组数)
    """
    group_counts: Counter[tuple[str, int]] = Counter()
    class_players: dict[int, set[str]] = defaultdict(set)

    for player_id, (_, label) in zip(dataset.player_ids, dataset.samples):
        group_counts[(player_id, label)] += 1
        class_players[label].add(player_id)

    weights: list[float] = []
    for player_id, (_, label) in zip(dataset.player_ids, dataset.samples):
        group_size = group_counts[(player_id, label)]
        player_groups_in_class = max(1, len(class_players[label]))
        weight = 1.0 / (group_size * player_groups_in_class)
    ...
    print("\n" + "=" * 70)
    print("Player-Class Balanced Sampling")
    print("=" * 70)
    for class_id, class_name in enumerate(CLASS_NAMES):
        players = class_players.get(class_id, set())
        sample_count = sum(1 for _, label in dataset.samples if label == class_id)
        print(
            f"{class_name:<24} videos={sample_count:<4} "
            f"players={len(players):<3}"
        )
...
class PlayerClassBalancedTrainer(Trainer):
...
# ============================================================
# 11. Stage2：Layer-wise Learning Rate + Multi-Clip Validation
# ============================================================
...
class LayerwiseMultiClipTrainer(PlayerClassBalancedTrainer):
    def __init__(
        self,
        *args,
    ...
    def evaluate(
        ...
        """
        训练期间每个epoch直接使用3-Clip Validation。
        因为Trainer的best-model选择与EarlyStopping均读取eval_macro_f1，
        所以这里返回的3-Clip Macro-F1会直接决定最佳checkpoint。
        """
        ...
        with torch.inference_mode():
            for video_path, true_label in self.multi_clip_eval_dataset.samples:
                ...
                clips = temporal_multiview_sample_frames(
                    ...
                    num_clips=self.multi_clip_num_clips,
                ...
                # V3默认采用Logits Mean -> Softmax/Argmax
        ...
        print(
            f"\n[3-Clip VAL] epoch={self.state.epoch} "
...
def evaluate_multi_clip(
    ...
    print("\n" + "=" * 70)
    print(f"{num_clips}-Clip Temporal Multi-View 测试")
    print("=" * 70)
...
def main() -> None:
    ...
    print("=" * 70)
    print("VideoMAE V3 - Basketball Shot Type Classification")
    print("=" * 70)
    ...
    stage1_trainer = PlayerClassBalancedTrainer(
        model=model,
        args=stage1_args,
    ...
    # ========================================================
    # Stage 2：Layer-wise LR + 3-Clip Validation
    # ========================================================
    ...
    print("\n" + "=" * 70)
    print("Stage 2：全量微调 + Layer-wise LR + 3-Clip Validation")
    print("=" * 70)
    ...
    print(f"最佳3-Clip VAL Macro-F1：{stage2_trainer.state.best_metric}")
    ...
    # ========================================================
    # 测试：保留Single-Clip用于对照 + 5-Clip作为主结果
    # ========================================================
    ...
    comparison = pd.DataFrame(
        [
            ...
            {
                "method": f"{NUM_TEST_CLIPS}_clip_temporal_multiview",
