...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "videomae_v7"
...
# ============================================================
# 2. V7训练参数
#    严格消融版本：以V5为基线，只改变Stage2解冻范围。
#    ① 时间采样完全恢复V5：训练90~100%连续窗口，验证/测试中心3-Clip
#    ② Loss、Sampler、增强、学习率、EarlyStopping均保持V5
#    ③ Stage2不再全量解冻，仅解冻VideoMAE最后4个Transformer Blocks
#    ④ 同时训练Backbone最终归一化层、fc_norm与classifier
#    ⑤ 锁定V5 split SHA256，保证V5/V7严格可比
#    ⑥ 继续保存每个Stage2验证Epoch诊断并检测类别塌缩
# ============================================================
RANDOM_SEED = 42
...
# V7恢复Hard Cross Entropy，不做Label Smoothing/Ordinal Soft Target
...
# Stage 2：仅微调顶部4个Transformer Blocks
# 为了做严格消融，学习率继续沿用V5。
STAGE2_EPOCHS = 25
STAGE2_UNFREEZE_TOP_N_BLOCKS = 4
CLASSIFIER_LR = 5e-5
TOP_ENCODER_LR = 2e-5
# 以下V5参数保留在配置/摘要中用于实验记录；V7冻结对应低层，因此不会参与优化。
MID_ENCODER_LR = 1e-5
...
# 每次正式实验清理V7旧stage输出，避免误恢复旧checkpoint
CLEAN_OLD_STAGE_OUTPUTS = True

# V5固定数据划分指纹。V7若检测到Train/Val/Test成员或标签变化，直接停止。
...
# ============================================================
# 8. V7：Player-Class软平衡 + Hard Cross Entropy
# ============================================================
def build_player_class_sample_weights(dataset: BasketballVideoDataset) -> torch.DoubleTensor:
    ...
    print("\n" + "=" * 70)
    print("V7 Player-Class Soft Sampling")
    print("=" * 70)
...
class PlayerBalancedTrainer(Trainer):
    """V7：Hard Cross Entropy由Transformers默认模型loss负责，仅覆盖训练Sampler。"""
...
def unfreeze_top_blocks_for_stage2(
    ...
    """
    V7严格消融：先冻结全部参数，只解冻最后top_n_blocks个Transformer Block、
    Backbone最终归一化层、fc_norm和classifier。

    返回：
        (first_unfrozen_block, total_encoder_blocks)
    """
    for parameter in model.parameters():
        parameter.requires_grad = False
    ...
    # 顶部Block后面的Backbone归一化层应随顶部语义层一起适配。
    ...
    # 只训练顶部4个Block时显存压力明显降低，不启用gradient checkpointing，
    # 避免额外计算开销；这不改变可训练参数集合。
    try:
        model.gradient_checkpointing_disable()
    except Exception:
        pass
...
# ============================================================
# 11. Stage2：Top-4 Layer-wise Learning Rate + V7中心加权Multi-Clip Validation
# ============================================================
...
class LayerwiseMultiClipTrainer(PlayerBalancedTrainer):
    def __init__(
        ...
        **kwargs,
        ...
        if self.multi_clip_num_clips != 3:
            raise ValueError("V7中心加权Multi-Clip固定使用3个Clip")
        if len(self.multi_clip_weights) != 3:
            raise ValueError("V7 CENTER_CLIP_WEIGHTS必须包含3个权重")
    ...
    def evaluate(
        ...
        """
        V7训练期间每个Epoch使用中心附近3-Clip Validation：
        ...
        print(
            f"\n[V7 3-Clip Weighted VAL] epoch={self.state.epoch} "
...
def evaluate_multi_clip(
    ...
    print("\n" + "=" * 70)
    print("V7 3-Clip Center-Weighted 测试")
    print("=" * 70)

    if num_clips != 3:
        raise ValueError("V7测试固定使用3个中心附近Clip")
...
def main() -> None:
    ...
    print("=" * 70)
    print("VideoMAE V7 - Basketball Shot Type Classification")
    print("=" * 70)
    ...
    if EXPECTED_SPLIT_SIGNATURE and split_signature != EXPECTED_SPLIT_SIGNATURE:
        raise RuntimeError(
            "\nV7检测到Train/Val/Test数据划分发生变化，停止实验。\n"
            f"期望指纹：{EXPECTED_SPLIT_SIGNATURE}\n"
            f"当前指纹：{split_signature}\n"
            "请恢复V5使用的同一套split后再运行，以保证V5/V7严格可比。"
        )
    print(f"Split SHA256已验证：{split_signature}")
    ...
    # ========================================================
    # Stage 2：V7唯一核心改动——仅解冻Top 4 Blocks
    # 时间采样、3-Clip Validation、Loss、Sampler、LR均保持V5。
    # ========================================================
    ...
    print("\n" + "=" * 70)
    print("Stage 2：Top 4 Blocks微调 + V5时间采样 + 中心加权3-Clip Validation")
    print("=" * 70)
    ...
    print(f"Train Temporal Ratio：{TRAIN_TEMPORAL_MIN_RATIO:.2f}~{TRAIN_TEMPORAL_MAX_RATIO:.2f}")
    print(f"VAL Window Ratio：{VAL_TEMPORAL_WINDOW_RATIO:.2f}")
    print(f"VAL Center Shift Ratio：{CENTER_CLIP_SHIFT_RATIO:.2f}")
    ...
    print(f"最佳V7 Top4 + 3-Clip加权VAL Macro-F1：{stage2_trainer.state.best_metric}")
    ...
    # ========================================================
    # 测试：保留Single-Clip用于对照 + V7中心加权3-Clip作为主结果
    # ========================================================
